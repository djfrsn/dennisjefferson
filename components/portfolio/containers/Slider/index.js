...
class Slider extends Component {
  ...
  getSlideElements = dir => {
    ...
    if (dir === "right") {
      elements.current =
        elements.current < elements.itemsTotal - 1 ? elements.current + 1 : 0;
    } else {
      // keeps track of urrent index & handles looping of slides
      elements.current =
        elements.current > 0 ? elements.current - 1 : elements.itemsTotal - 1;
    }
   ...
   */
  onViewDetails = callee => {
    // using fat arrow to avoid having to bind 'this' in the constructor. *only required for your custom methods!
  ...
  applyTransforms = component => {
    ...
    const componentAreaSize = {
      width: componentArea.offsetWidth,
      height: componentArea.offsetHeight
    };
    // const componentOffset = componentArea.getBoundingClientRect();
    const scaleVal =
      componentAreaSize.width / componentAreaSize.height <
      window.innerWidth / window.innerHeight
        ? window.innerWidth / componentAreaSize.width
        : window.innerHeight / componentAreaSize.height;
  ...
  animate(options) {
    ...
    dynamics.animate(
      options.currentEl,
      {
        ...
        translateX:
          options.dir === "right"
            ? (-1 * options.currentEl.offsetWidth) / 2
            : options.currentEl.offsetWidth / 2,
        rotateZ: options.dir === "right" ? -10 : 10
      },
      {
        ...
        complete: () => {
          if (!this.willUnmount) {
            dynamics.css(options.itemCurrent.slide, {
              opacity: 0,
              visibility: "hidden"
            });
          }
    ...
    dynamics.animate(
      options.currentTitleEl,
      { translateX: options.dir === "right" ? -250 : 250, opacity: 0 },
      {
    ...
    dynamics.css(options.nextEl, {
      opacity: 0,
      translateX:
        options.dir === "right"
          ? options.nextEl.offsetWidth / 2
          : (-1 * options.nextEl.offsetWidth) / 2,
      rotateZ: options.dir === "right" ? 10 : -10
    });
    ...
    dynamics.css(options.nextTitleEl, {
      translateX: options.dir === "right" ? 250 : -250,
      opacity: 0
    });
  ...
  render() {
    ...
    return (
      <section
        className={cx("slider")}
        style={{ opacity: this.props.opaque ? 0.1 : 1 }}
