...
class ProfileCard extends Component {
  ...
  componentDidMount() {
    this.ProfileCardParent.addEventListener(
      "animationend",
  ...
  componentWillReceiveProps(nextProps) {
    // Runs when parent changes 'show' prop
    this.setState({
      show: nextProps.show,
  ...
  componentWillUnmount() {
    this.ProfileCardParent.removeEventListener(
      "animationend",
      this.onEndTransition
    );
  }
  ...
  render() {
    const cardheaderBg = {
      background: `url(${this.props.baseurl}${this.props.author.header_bg})`
    };
    const avatar = `${this.props.baseurl}${this.props.author.avatar}`;
    // Images above, animations below
    const profile = this.state.show
      ? { zIndex: 1000, visibility: "visible" }
      : { zIndex: 0, visibility: "hidden" };
    const profileSection = this.state.show
      ? { zIndex: 1000, visibility: "visible" }
      : { zIndex: 0, visibility: "hidden" };
    ...
    const profileCard = this.state.show
      ? { visibility: "visible", zIndex: 1000 }
      : { visibility: "hidden", zIndex: 0 };
    const closeProfileButton =
      !this.state.bounceIn && this.state.show
        ? { visibility: "visible" }
        : { visibility: "hidden" };
    return (
      <section
      ...
      >
        <div
          className={profileCardParent}
          style={profileSection}
          ref={ref => (this.ProfileCardParent = ref)}
        >
          <div
            className={cx("card", "hovercard", "profile_card")}
            style={profileCard}
          >
            <a
              className={cx("bp-icon", "bp-icon-close", "close_profile")}
              onClick={this.closeProfile}
              style={closeProfileButton}
            >
              <span />
            </a>
            ...
            <div className={cx("bottom")}>
              <a
                className={cx("btn", "btn-primary", "btn-twitter", "btn-sm")}
